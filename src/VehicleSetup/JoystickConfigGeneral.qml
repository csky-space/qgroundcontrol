    ...
    Column {
        ...
        Row {
            Rectangle {
                ...
                border.width:       1
                radius:             ScreenTools.defaultFontPixelWidth * 0.5
                width:              axisRect.width * 2.0
                height:             childrenRect.height

                Connections {
                    target: controller
                    onRcMappingValueChanged: (axis, value) => {
                        if (axisMappingRepeater.itemAt(axis)) {
                            axisMappingRepeater.itemAt(axis).axis.axisValue = value
                        }
                    }
                }
                Column {
                    ...
                    Repeater {
                        id:             axisMappingRepeater
                        model:          _activeJoystick ? _activeJoystick.rcMappingsCount : 0
                        width:          parent.width
                        ...
                        Row {
                            ...
                            QGCComboBox {
                                ...
                                model:                      _activeJoystick ? _activeJoystick.rcMappingOptions : []
                                currentIndex:               _activeJoystick ? _activeJoystick.rcMappingIndexes[index] : index;
                                sizeToContents:             true
                                anchors.verticalCenter:     parent.verticalCenter

                                onCurrentIndexChanged: () => {
                                    if (_activeJoystick) {
                                        let indexes = _activeJoystick.rcMappingIndexes;
                                        indexes[index] = currentIndex;
                                        _activeJoystick.setRCMappingIndexes(indexes);
                                    }
                                }
                            }
                            AxisMonitor {
                            ...
                            QGCCheckBox {
                                ...
                                text:                       qsTr("Inverse")
                                anchors.verticalCenter:     parent.verticalCenter
                                checked:                    _activeJoystick ? _activeJoystick.rcMappingInverses[index] : false;

                                onClicked: {
                                    if (_activeJoystick) {
                                        let inverses = _activeJoystick.rcMappingInverses;
                                        inverses[index] = checked;
                                        _activeJoystick.setRCMappingInverses(inverses);   
                                    }
                                }
                            }
                        }
